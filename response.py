...
def get_app_response(prediction):
    ...
    user_message = f"Please provide general information, causes, specific remedies, and where they appear for '{prediction}' in 175 words - Go pretty in depth. In less than 30 words, please include a sentence that mentions in some way that the model uses image detection and isn't perfect so they should also check out some diseases similar to '{prediction}' of your knowledge that look similar and may get confused by the model."
    messages = [
